...
PolygonHierarchy::PolygonHierarchy(cv::Mat& img) : data(), subTree() {
  ...
  cv::findContours(img, contoursFine, hierarchy, cv::RetrievalModes::RETR_TREE,
  ...
  for (size_t i = 0; i < contoursFine.size(); i++) {
    ...
    // std::cout << "Coarsening Factor: " <<
    // contoursFine[i].size()/contoursCoarsened[i].size() << std::endl;
  }
...
PolygonHierarchy::PolygonHierarchy(std::vector<cv::Point> contour,
                                   std::vector<PolygonHierarchy> subTreeParam)
    : id(false), data(contour), subTree(subTreeParam) {}

std::vector<PolygonHierarchy> PolygonHierarchy::createSubTree(
...
std::vector<cv::Point> PolygonHierarchy::getData() { return data; }

std::vector<PolygonHierarchy> PolygonHierarchy::getSubTree() { return subTree; }
}  // namespace img
}  // namespace sfcpp
